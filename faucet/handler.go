...
func verifyChainIDs(config *Config) error {
	...
	if len(notChainIDs) > 0 {
		return fmt.Errorf("chains %s not found in chain faucet directory", strings.Join(notChainIDs, ","))
	}
...
func (a *AppServer) GetChain(ctx context.Context, requestChain *pb.RequestChain) (*pb.ChainFaucet, error) {
	chainID := requestChain.Chain

	filename := filepath.Join(a.config.ChainFaucet, chainID, "chain.json")
	chain := &pb.ChainFaucet{}
...
func (a *AppServer) GetChainAssets(ctx context.Context, requestChain *pb.RequestChain) (*pb.ResponseChainAssets, error) {
	...
	filename := filepath.Join(a.config.ChainFaucet, chainID, "assetlist.json")
