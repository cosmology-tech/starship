...
do
  ...
  RELAYER_CLI_KEY_NAME="$(jq -r '.relayers[$i].name' $KEYS_CONFIG)"
  echo "Adding relayer-cli key.... $RELAYER_CLI_KEY_NAME"
  jq -r ".relayers[$i].mnemonic" $KEYS_CONFIG | $CHAIN_BIN keys add $RELAYER_CLI_KEY_NAME --recover --keyring-backend="test"
