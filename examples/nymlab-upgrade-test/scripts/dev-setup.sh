...
if [ -n "$(kubectl config view -o jsonpath='{.users[?(@.name == "kind-starship")]}')" ] 
then
    color green "Kubernetes Ready!!!"
else
    kind create cluster --name starship
    kubectl config use-context kind-starship
    color green "Init Kubernetes Context"
fi